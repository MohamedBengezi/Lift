...
export const precomputePlans = functions.https.onRequest(async (req, res) => {
  ...
  for (let u = 0; u < users.length; u++) {
    promises.push(buildPostRecommendations(G2, u, users, posts));
    promises.push(buildPlanRecommendations(G1, u, users, plans));
  }
  ...
  res.send("Completed recommendations");
});
...
function buildPlanRecommendations(G: Graph, u: number, users: any, plans: any) {
  ...
  let deg = 4;
  loop1: while (deg < 8) {
    loop2: for (let i = numUsers; i < numUsers + numPlans; i++) {
      if (
        b.hasPathTo(i) &&
        getDegree(b.pathTo(i) || new Stack()) == deg &&
        !recs.includes(i)
      ) {
        if (recs.length < 100) {
          recs.push(i);
  ...
  for (let i = 0; i < recs.length - 1; i++) {
    for (let j = 0; j < recs.length - i - 1; j++) {
      if (
        plans[recs[j] - users.length]!.rating -
          getDegree(b.pathTo(recs[j]) || new Stack()) >
        plans[recs[j + 1] - users.length]!.rating -
          getDegree(b.pathTo(recs[j + 1]) || new Stack())
      ) {
        let temp = recs[j];
        recs[j] = recs[j + 1];
        recs[j + 1] = temp;
  ...
  return admin.firestore().collection("users").doc(uid).update(info);
}
...
function buildPostRecommendations(G: Graph, u: number, users: any, posts: any) {
  ...
  let deg = 4;
  loop1: while (deg < 8) {
    loop2: for (let i = numUsers; i < numUsers + numPosts; i++) {
      if (
        b.hasPathTo(i) &&
        getDegree(b.pathTo(i) || new Stack()) == deg &&
        !recs.includes(i)
